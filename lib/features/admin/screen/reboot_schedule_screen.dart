...
class RebootScheduleScreen extends StatefulWidget {
  const RebootScheduleScreen({super.key});

  @override
  State<RebootScheduleScreen> createState() => _RebootScheduleScreenState();
}

class _RebootScheduleScreenState extends State<RebootScheduleScreen> {
  static const _rebootChannel = MethodChannel('reboot_channel');

  late RebootSchedule _schedule;
  bool _saving = false;
  ...
  void initState() {
    super.initState();
    _schedule = ServiceLocator().rebootScheduleService.getSchedule();
  }
  ...
  Future<void> _save() async {
    if (_schedule.type == RebootScheduleType.weekly && _schedule.weekdays.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('요일을 하나 이상 선택해주세요.')),
      );
      return;
    }
    if (_schedule.type == RebootScheduleType.specificDate &&
    ...
    await ServiceLocator().rebootScheduleService.saveSchedule(_schedule);

    try {
      if (_schedule.enabled) {
        final payload = jsonEncode(_schedule.toJson());
        await _rebootChannel.invokeMethod('scheduleReboot', payload);
        FlutterErrorLogger.system(
          '[시스템] 자동 재부팅 스케줄 설정: ${_scheduleDescription()}',
          errorCode: 'SYS-010',
          severity: 'INFO',
        );
      } else {
        await _rebootChannel.invokeMethod('cancelReboot');
        FlutterErrorLogger.system(
          '[시스템] 자동 재부팅 스케줄 취소',
          errorCode: 'SYS-011',
          severity: 'INFO',
  ...
  String _scheduleDescription() {
    ...
    switch (_schedule.type) {
      case RebootScheduleType.daily:
        return '매일 $time';
      case RebootScheduleType.weekly:
        final days = _schedule.weekdays
            .map((d) => _weekdayLabels[d - 1])
            .join(', ');
        return '요일별($days) $time';
      case RebootScheduleType.specificDate:
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (_schedule.enabled) ...[
                      ...
                      _card(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _typeOption(RebootScheduleType.daily, '매일', labelFont),
                            _typeOption(RebootScheduleType.weekly, '요일별', labelFont),
                            _typeOption(RebootScheduleType.specificDate, '특정 날짜', labelFont),
                          ],
                        ),
                      ),
                      SizedBox(height: _rs(24)),
                      ...
                      // 요일 선택 (weekly)
                      if (_schedule.type == RebootScheduleType.weekly)
                        _card(
                          cardRadius,
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                      ...
                      _card(
                        ...
                        child: GestureDetector(
                          ...
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text('재부팅 시간', style: _boldStyle(cardFont)),
                              Text(
                                '${_schedule.hour.toString().padLeft(2, '0')}:${_schedule.minute.toString().padLeft(2, '0')}',
                                style: TextStyle(
                                  fontFamily: AppTextStyles.bodyFontFamily,
  ...
  Widget _typeOption(RebootScheduleType type, String label, double fontSize) =>
      GestureDetector(
